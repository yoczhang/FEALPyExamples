...
for i in range(maxit):
    ...
    # save data
    # './results/'
    fname = '../results/' + 'vem' + str(i) + '.space'
    f = open(fname, 'wb')
    pickle.dump([vem, qtree, data], f)

    fname = '../results/' + 'error' + '-' + str(i) + '.data'
    f = open(fname, 'wb')
    ...
    plt.savefig('../results/' + str(i) + '-solution.png')
    plt.close()

    fig1 = plt.figure()
    axes = fig1.gca()
    mesh.add_plot(axes)
    plt.savefig('../results/' + str(i) + '-mesh.png')
    plt.close()
