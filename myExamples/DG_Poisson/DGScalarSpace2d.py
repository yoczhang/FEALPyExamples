...
class DGScalarSpace2d(ScaledMonomialSpace2d):
    ...
    def averjumpIn_matrix(self):  # the average-jump matrix at interior edges
        p = self.p
        mesh = self.mesh
    ...
    def jumpjumpIn_matrix(self):  # the jump-jump matrix at interior edges
        ...
        J_matrix = np.array([Jmm, -Jmp, -Jpm, Jpp])  # J_matrix.shape: (4,NInE,ldof,lodf)
        ...
        J_matrix = csr_matrix((J_matrix.flat, (row.flat, col.flat)), shape=(gdof, gdof))
        return J_matrix

    def jumpjumpDir_matrix(self):  # the jump-jump matrix at Dirichlet edges
        p = self.p
        mesh = self.mesh
        node = mesh.entity('node')
        ...
        edge = mesh.entity('edge')
        edge2cell = mesh.ds.edge_to_cell()
        edgeArea = mesh.mesh.edge_length()
        ...
        # Construct the jump-matrix
        Jmm = csr_matrix((Jmm.flat, (rowmm.flat, colmm.flat)), shape=(gdof, gdof))
        return Jmm
